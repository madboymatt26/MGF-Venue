...
$wpdb->query( "CREATE TABLE IF NOT EXISTS {$barrier_table} (barrier_key VARCHAR(64) NOT NULL, arrived INT NOT NULL DEFAULT 0, PRIMARY KEY (barrier_key)) ENGINE=InnoDB" );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$barrier_table} WHERE barrier_key = %s", $invoice_ref ) );
$wpdb->insert( $barrier_table, array( 'barrier_key' => $invoice_ref, 'arrived' => 0 ) );
