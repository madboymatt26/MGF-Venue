...
<div class="wrap nms-admin-wrap">
    ...
    <?php else : ?>
        ...
        <section class="postbox" style="padding:16px;">
            ...
            <?php foreach ( $invoices as $invoice ) : $balance = MBS_Billing_Ledger::balance_minor( $invoice ); $document_id = $invoice_documents[ (int) $invoice->id ] ?? null; ?>
                <div class="mbs-invoice-card">
                    <div class="mbs-invoice-card__header">
                        <?php if ( $document_id ) : ?>
                            <button type="button" class="mbs-invoice-card__ref mbs-view-invoice-btn" data-document-id="<?php echo (int) $document_id; ?>" data-invoice-ref="<?php echo esc_attr( $invoice->invoice_ref ); ?>"><?php echo esc_html( $invoice->invoice_ref ); ?></button>
                        <?php else : ?>
                    ...
                    <div class="mbs-invoice-card__actions">
                        <?php if ( $document_id ) : ?>
                            <button type="button" class="button mbs-view-invoice-btn" data-document-id="<?php echo (int) $document_id; ?>" data-invoice-ref="<?php echo esc_attr( $invoice->invoice_ref ); ?>">View invoice</button>
                            <a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=mbs_invoice_document&document_id=' . (int) $document_id . '&format=pdf&mode=issued&nonce=' . wp_create_nonce( 'mbs_invoice_document_nonce' ) ) ); ?>" class="button" target="_blank" rel="noopener">Download PDF</a>
                        <?php else : ?>
...
<div id="mbs-invoice-modal" class="mbs-modal" style="display:none;" role="dialog" aria-modal="true" aria-labelledby="mbs-invoice-modal-title">
    <div class="mbs-modal__backdrop"></div>
    <div class="mbs-modal__content">
        <div class="mbs-modal__header">
            <h2 class="mbs-modal__title" id="mbs-invoice-modal-title">Invoice</h2>
        ...
        <div class="mbs-modal__footer">
            <a href="#" class="button mbs-modal__download" target="_blank" rel="noopener">Download PDF</a>
